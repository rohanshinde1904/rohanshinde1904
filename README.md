# 👋 Hi, I'm Rohan 

💡 Data Engineer | PySpark | Databricks | Big Data | Azure | Python | SQL | Shell | Unix

---

## 🚀 About Me
- 4+ years of experience in **Data Engineering, ETL, and Big Data systems**.
- Skilled in **PySpark, Databricks, SQL, ADF, Delta Lake**.

---

## 🛠️ Tech Stack
**Languages:** Python, SQL, Shell  
**Big Data:** PySpark, Apache Spark, Databricks, Delta Lake  
**Cloud:** Azure (ADF, Synapse, Databricks)
**Databases:** Oracle, MySQL, SQL Server  
**Other:** Git, Linux, Docker (basic)

---

## 📌 Projects (Work in Progress 🚧)
I’m currently working on end-to-end data engineering projects. Here’s what’s coming up soon:
  
1. **Sales Anomaly Detection (PySpark + Databricks ML)**  


👉 Stay tuned – I’ll upload repos with full READMEs, architecture diagrams, and notebooks.

---

## 📫 Connect with Me
- 💼 https://www.linkedin.com/in/rohan-shinde-00b135171/
- 📧 shinderohan1904@gmail.com
